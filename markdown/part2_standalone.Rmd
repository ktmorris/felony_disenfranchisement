...
Although Figures \ref{fig:to-16} and \ref{fig:to-18} indicate that the length of time an individual has been off parole is correlated with her propensity to vote, that relationship is far weaker in the short term. Figure \ref{fig:to-18-recent} indicates that parole discharge date and turnout rates are not correlated when we limit the analysis to individuals discharged in 2017 or 2018.
...
This lack of correlation between discharge date and turnout indicates that we can conceptualize Executive Order as a natural experiment. We can expect that, in the absence of Executive Order 181, individuals discharged from parole before and after the policy went into effect would have turned out at comparable rates. Any observed difference in turnout rates, therefore, can be attributed to the implementation of the policy.
...
In Table \ref{tab:to-18-logit}, I present the results of an individual-level logistic regression exploring turnout in 2018 among individuals who were discharged on or after May 2121^st^, 2018, the date when the policy went into full effect. It includes all individuals discharged from parole between January 1^st^, 2017, through October 10^th^, 2018 (the registration deadline in New York State).
...
stargazer(model1, model2, model3,
          header = F,
          type = "latex",
          covariate.labels = c("D(Discharged After EO 181)", "D(Male)", "Age (Years)", "Counts in Most Recent Sentence",
                               "Time Spent on Parole (Years)"),
          dep.var.labels = c("Cast Ballot in 2018 Election"),
...
Table \ref{tab:to-18-logit} indicates that Executive Order was successful at increasing turnout among all formerly disenfranchised individuals. The above analysis, however, likely understates the impact of the Executive Order on individuals who were in fact eligible to cast a ballot. As Figure \ref{fig:restoration-chart} indicates, not all individuals discharged after the Executive Order went into effect had their rights restored. Individuals who were discharged after the policy was changed but but did not have their rights restored can be conceptualized as "non-compliers" --- although they were "assigned" to the treatment group (were discharged after the policy changed), they did not receive the treatment (have their rights formally restored). Some of these non-compliers were likely non-citizens, who could not be granted voting rights. Others may have been re-arrested, which disqualified them from having rights restored. Both non-citizens and individuals who have been recently arrested are unlikely to vote. "Compliance," therefore, is almost certainly correlated with with an individual's likelihood of participating in the 2018 election. As such, the estimated treatment effect on the treated --- on individuals who were likely eligible to cast a ballot on election day --- requires an instrumental variables approach. 

In the models below, a dummy indicating whether an individual was discharged from parole after the Executive Order went into effect (i.e., was in the "intend-to-treat" group) is used as an instrument for whether an individual's rights were actually restored (i.e., "complied" with the treatment). Table \ref{tab:iv-models} shows the second-stage estimation results, estimated using an OLS sepcification.^[Although the dependent variable remains dichotomous (as do both the independent variable of interest and the instrument), @Angrist2008 recommends using OLS in instrumental variables analyses even under such conditions.]
