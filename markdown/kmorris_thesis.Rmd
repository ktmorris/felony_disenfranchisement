...
Much of the literature has discussed whether felony disenfranchisement is particularly demobilizing for eligible Black voters. I therefore include models which explore any potential difference in treatment effect in neighborhoods where a higher share of the population is Black. Table \ref{tab:match-reg} presents the results of these regression models.
```{r tract-reg-chunk, echo = F, results = "asis", message = F}
tract_reg <- readRDS("./temp/match_reg_tract.rds")
tract_reg_black <- readRDS("./temp/match_reg_tract_nhb.rds")
bg_reg <- readRDS("./temp/match_reg_block_group.rds")
bg_reg_black <- readRDS("./temp/match_reg_block_group_nhb.rds")
...
stargazer(bg_reg, bg_reg_black, tract_reg, tract_reg_black,
          ...
          header = F,
          type = "latex",
          covariate.labels = c("D(Neighborhood Lost a Voter)", "Share Non-Hispanic Black", "D(Lost Voter) X Share Black", "Constant"),
          ...
          omit.stat = c("f", "ser"))
```
When neighborhoods are measured at the block group level, a lost voter is negatively associated with turnout in the 2017 election. In block groups with lost voters, turnout was on average 0.8 percent lower than in comparable block groups without lost voters. This decrease, however, appears to be entirely concentrated within Black block groups. When the dummy identifying neighborhoods with lost voters is interacted with the share of the neighborhood that is Non-Hispanic Black, the basic treatment dummy becomes insignificant (p = 0.24). The coefficient on the interaction between treatment and share Black indicates that neighborhoods that are largely Black saw turnout that was as much as *3 percent lower* than similar neighborhoods without lost voters. These variables are largely insignificant at the census tract level. This is not particularly surprising. If lost voters have the largest depressive effect on family members and close neighbors, it makes sense that the effect of losing a voter is locally concentrated.

### Testing Intensity Effects {-}

Matching methodologies, of course, only allow us to test the effect of being treated - here, losing a voter for the 2017 election. The model above does not allow for different effects on turnout based on *how many* voters a neighborhood lost. In Table \ref{tab:trad-reg} below, I adopt a standard regression to investigate whether lost voters are associated with lower turnout rates in the 2017 election.

```{r trad-reg-chunk, echo = F, results = "asis", message = F}
tract_reg <- readRDS("./temp/tract_model_reg_nosquare.rds")
tract_reg_sq <- readRDS("./temp/tract_model_reg_square.rds")
bg_reg <- readRDS("./temp/bg_model_reg_nosquare.rds")
bg_reg_sq <- readRDS("./temp/bg_model_reg_square.rds")

stargazer(bg_reg, bg_reg_sq, tract_reg, tract_reg_sq,
          column.separate = c(2, 2),
          column.labels = c("Block Group Level", "Tract Level"),
          header = F,
          type = "latex",
          covariate.labels = c("Lost Voters", "Lost Voters X Share Black", "Median Income (Thousands of Dollars)", "Percent Latino", "Percent Non-Hispanic Black", "Percent Non-Hispanic White",
