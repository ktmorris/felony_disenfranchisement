...
To test whether there is a relationship between lost voters and voter file undercount, I examine the undercount rate at the precinct level. The voter file indicates the home precinct of each voter, and the Board of Elections publishes election results at the precinct level. Because the census does not report statistics at the precinct level, I use block group information instead. Where precincts cross block group lines, the precinct is assigned the characteristics of all block groups in which it is located weighted by the number of voters from each block group. The undercount rate is calculated as the number of votes derived from the voter file divided by the number reported in BOE results.
