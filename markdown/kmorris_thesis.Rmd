...
Figure \ref{fig:to-18} displays the share of individuals who were discharged from parole in each month and cast a ballot in the 2018 elections. It does not control for any sort of individual level characterisitcs. In \ref{tab:to-18-logit}, I present the results of an individual-level regression exploring the turnout in 2018 among individuals whose rights were restored. Because turnout is a binary dependent variable, this is a logistic regression. It includes all individuals discharged from parole between January 1^st^, 2012, through October 10^th^, 2018 (the registration deadline in New York State).^[This of course ignores the individuals who were still on parole and cast a ballot in the 2018 election. However, for this exercise, I limit the analysis group to individuals who would have been eligible to vote on election day even if Governor Cuomo had not signed Executive Order 181.]
...
stargazer(iv1, iv2, iv3,
          ...
          notes = c("Table reports p-values for Wu-Hausman and",
                    "Weak Instruments tests.",
                    "D(Discharged from Parole on or after May",
                    "21, 2018) instrument for D(Rights Restored)"))
```

Table \ref{tab:iv-models} indicates that using D(Discharged from Parole on or After May 21) is likely warranted. The p-values of the Wu-Hausman tests hover around 0.05, while the Weak Instruments test is highly significant in each model.
