...
After matching neighborhoods, I use a simple regression to test whether turnout in the 2017 mayoral election was different in areas with lost voters. Census tract and block group level turnout rates are calculated using the geocoded voter file. Each voter's record indicates whether the voter participated in the 2017 general election,^[The New York State voter file is not without errors. For instance, the voter file erroneously does not record the author as participating in the 2017 mayoral election. However, insofar as these errors are not geographically correlated, there is little reason to believe they impact the results of this analysis. Unfortunately, testing the distribution of these errors is not possible.] which are then aggregated to estimate the number of ballots cast in each neighborhood. The number of ballots cast is divided the voting age population.
