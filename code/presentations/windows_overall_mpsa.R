...
###
estimates <- estimates %>% 
  filter(!(grepl("race|felony_", coef)),
         coef != "(Intercept)") %>% 
  mutate(coef = ifelse(coef == "restoredTRUE", "Treated", coef),
         coef = ifelse(coef == "parole_time", "Years on Parole", coef),
         coef = ifelse(coef == "maleTRUE", "Male", coef),
         coef = ifelse(coef == "age", "Age", coef))

estimates$coef <- factor(estimates$coef, levels = rev(c("Treated", "Age", "Male", "Years on Parole")))
...
p <- ggplot(data = estimates) + theme_bw() +
  ggstance::geom_pointrangeh(aes(y = coef, x = estimate, 
  ...
  labs(caption = "Notes: \"Treated\" is instrumented by whether individual came off parole after executive order went into effect. \nModels also include felony class and race / ethnicity fixed effects.") +
  theme(plot.caption = element_text(hjust = 0, size = 8),
        text = element_text(size = 18),
        axis.text = element_text(size = 18))

saveRDS(p, "./temp/window_change_plot_mpsa.rds")

##################################

iv1 <- as.data.frame(confint_robust(ivreg(v2018 ~ restored + male +
                                            age +
                                            felony_a + felony_b + felony_c + felony_d + felony_e + parole_time | . -restored + finished_post,
                                          data = filter(parolees, race == "WHITE",
                                                        year(parole_status_date) >= 2017)), level = 0.95,
                                    HC_type = "HC1")) %>% 
  add_rownames(var = "coef") %>% 
  #filter(coef == "restoredTRUE") %>% 
  rename(lb = `2.5 %`,
...
iv2 <- as.data.frame(confint_robust(ivreg(v2018 ~ restored + male +
                                            age +
                                            felony_a + felony_b + felony_c + felony_d + felony_e + parole_time | . -restored + finished_post,
                                          data = filter(parolees, race != "WHITE",
                                                        year(parole_status_date) >= 2017)), level = 0.95,
...
iv3 <- as.data.frame(confint_robust(ivreg(v2018 ~ restored + male +
                                            age +
                                            felony_a + felony_b + felony_c + felony_d + felony_e + parole_time | . -restored + finished_post,
                                          data = filter(parolees, race == "BLACK",
                                                        year(parole_status_date) >= 2017)), level = 0.95,
                                    HC_type = "HC1")) %>% 
  add_rownames(var = "coef") %>% 
  #filter(coef == "restoredTRUE") %>% 
...
estimates <- bind_rows(iv1, iv2, iv3)
estimates$window <- factor(estimates$window, levels = c("White", "Nonwhite", "Black"))
...
p <- ggplot(data = estimates) + theme_bw() +
  ...
  geom_vline(xintercept = 0,
             linetype = 2, 
             size = 0.25) +
  scale_color_manual(values = c("#000000", "#2b6f39", "#d38fc5", "navy"), name = "Race / Ethnicity") + 
  scale_shape_manual(values = shapes, name = "Race / Ethnicity") + theme() + 
  drop_y_gridlines() + theme(axis.title.y = element_blank(), 
                             panel.grid.major.x = element_line(linetype = "solid"),
                             text = element_text(family = "LM Roman 10"),
                             legend.pos = "right") + 
  scale_x_continuous(labels = percent) +
  xlab("Effect on Turnout (Percentage Point)") +
  labs(caption = "Notes: \"Treated\" is instrumented by whether individual came off parole after executive order went into effect. \nModels also include felony class fixed effects.") +
  theme(plot.caption = element_text(hjust = 0, size = 8),
        text = element_text(size = 18),
        axis.text = element_text(size = 18))

saveRDS(p, "./temp/race_change_plot_mpsa.rds")
