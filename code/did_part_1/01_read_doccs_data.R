...
### INDIVIDUALS WHO WERE INCARCERATED AS OF 2016 ELECTION
in_2016 <- all_doccs %>% 
  filter(gsub("[-]", "", latest_reception_date) < "20161108",
         (release_date > "2016-11-08" | is.na(release_date)),
         current_felon == T) %>% 
  group_by(din) %>% 
  filter(row_number() == 1) %>% 
  select(din, last, first, middle, dob, race)

### INDIVIDUALS ON PAROLE AS OF 2016 ELECTION
...
parolees_felons <- parolees %>% 
  filter(din %in% parolee_crimes,
         ...
         (status == "ACTIVE" | (status_date > "2016-11-08")))
...
## COMBINE TO FIND ALL DISENFRANCHISED AS OF ELECTION IN 2016
in_2016 <- bind_rows(in_2016, parolees_felons)
...
saveRDS(unique_16, "./temp/in_2016.rds")